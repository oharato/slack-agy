...
5. **インタラクティブ承認・選択肢スタンプ & ボタン連携 (Interactive Reaction & Button Approvals)**:
   - 危険コマンドやデプロイの承認（✅/❌）、実装方針の選択肢（1. 2. 3. や 1️⃣ 2️⃣ 3️⃣ や A. B. C.）を **自動検出**。
   - スレッド内にスタンプ（✅/❌ や 1️⃣/2️⃣/3️⃣）を自動付与し、さらに **Slack Block Kit のボタン** も同時に配置。
   - ユーザーがスタンプまたはボタンをクリックするだけで、文字入力なしで即座に次のターンが自律実行されます（5分タイムアウト保護付き）。
