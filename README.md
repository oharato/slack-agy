...
4. **Slack Block Kit Markdown ブロックによる表（Table）& リッチフォーマット表示**:
...
7. **放置 Worktree の自動ガベージコレクション (`WorktreeCleaner`)**:
   - タスク完了後に放置された Worktree を TTL（デフォルト: 7日間）に基づいて定期的に自動削除し、ディスク容量を保護。
8. **ポート開放不要の Socket Mode & systemd 常駐**:
