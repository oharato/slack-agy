...
## 3. Linux ホスト環境構築手順 (管理者セットアップ)

複数ユーザーが安全に共同開発を行うための Linux 側の初期設定手順です。

### 3.1 開発者グループ・サービス実行ユーザー・共有ディレクトリの作成

本システムは個人のユーザーアカウントではなく、**常駐サービス専用のシステムユーザー（例: `slack-agy`）** を作成して実行することを推奨します。
